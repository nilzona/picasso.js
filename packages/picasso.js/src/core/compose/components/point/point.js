...
function getPointSizeLimits(x, y, width, height, limits) {
  ...
  let maxSizePx = Math.min(
    xSpacePx.value
      * limits[xSpacePx.isBandwidth ? 'maxRelDiscrete' : 'maxRelExtent'],
    ySpacePx.value
      * limits[ySpacePx.isBandwidth ? 'maxRelDiscrete' : 'maxRelExtent']
  );
  let minSizePx = Math.min(
    xSpacePx.value
      * limits[xSpacePx.isBandwidth ? 'minRelDiscrete' : 'minRelExtent'],
    ySpacePx.value
      * limits[ySpacePx.isBandwidth ? 'minRelDiscrete' : 'minRelExtent']
  );
...
const component = {
  ...
  render({ data, style }) {
    const resolved = this.resolver.resolve({
      data,
      defaults: extend({}, DEFAULT_DATA_SETTINGS, style.item),
    ...
    const limits = extend(
      {},
      SIZE_LIMITS,
      this.userSettings.settings.sizeLimits
    );
    const points = resolved.items;
    const pointSize = getPointSizeLimits(
      resolved.settings.x,
      resolved.settings.y,
      width,
      height,
    ...
    return createDisplayPoints(
      points,
      this.rect,
      pointSize,
      this.userSettings.shapeFn || shapeFactory
    );
  }
};
